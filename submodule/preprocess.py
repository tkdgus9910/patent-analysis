...
def wisdomain_prep(data, drop_families = 1) :
    ...
    # 데이터 전처리
def wisdomain_prep_JP(data, drop_families = 1) :
    ...
    data = data.dropna(subset = ['IPC']).reset_index(drop = 1)
    
    
    data['year_application'] = data['date_application'].apply(lambda x : int(x.split('.')[0]) if str(x) != 'nan'  else x)
    
    
    data['IPC_sg'] = data['IPC'].apply(lambda x : x.split(', ') if str(x) != 'nan' else x)
    data['IPC_mg'] = data['IPC_sg'].apply(lambda x : list(([i.split('/')[0] for i in x ])))
    data['IPC_sc'] = data['IPC_mg'].apply(lambda x : list(([i[0:4] for i in x])))
    data['IPC_mc'] = data['IPC_sc'].apply(lambda x : list(([i[0:3] for i in x])))
    
    data['F-Term'] = data['F-Term'].apply(lambda x : x.split(', ') if str(x) != 'nan' else np.nan)
