...
class LDA_gensim :
    ...
    def tunning_passes(self, method = ['diversity', 'coherence']) :
        ...
        for passes in range(2, 21, 2):
            ...
            scores_df = pd.concat([scores_df, pd.DataFrame([score_dict])], ignore_index=1)
    ...
    def get_docByTopics(self) :
        ...
        for corp in corpus :
            ...
            docByTopics = pd.concat([docByTopics,pd.DataFrame([DICT])], ignore_index= 1)
    ...
    def get_wordByTopics(self) :
        ...
        for i in range(0, model.num_topics) :
            ...
            topic_word_df = pd.concat([topic_word_df,pd.DataFrame([DICT])], ignore_index=1)
    ...
    def get_TVByTime(self,
        ...
        for col in range(0, self.k) :
            ...
            topic_time_df = pd.concat([topic_time_df, pd.DataFrame([DICT])], ignore_index = 1)
    ...
    def get_summary_TVByTime(self, start_year, end_year) :
        
        topic_time_df = self.topic_time_df
        
        # 전체기간
        end_year_total = max(topic_time_df.index)
        start_year_total = min(topic_time_df.index)
        total_period = end_year_total - start_year_total +1
        
        # 관심기간
        recent_period = end_year-(start_year-1)
        
        # print(recent_year)
        
        topic_time_df_recent = topic_time_df.loc[(topic_time_df.index >= start_year) &
                                                 (topic_time_df.index <= end_year),:]
        
        topic_time_df_cum = topic_time_df.loc[(topic_time_df.index <= end_year),:]
        
        volume = topic_time_df_cum.apply(lambda x : np.sum(x))
        volume_recent = topic_time_df_recent.apply(lambda x : np.sum(x))
        
        cagr = topic_time_df.apply(lambda x : CAGR(x[start_year_total], x[end_year_total], total_period))
        cagr_recent = topic_time_df_recent.apply(lambda x : CAGR(x[start_year], x[end_year], recent_period))
        ...
        self.topic_time_df_summary = result[['topic','volume', 'volume_recent', 'CAGR', 'CAGR_recent']]
    ...
    def get_most_similar_doc2topic(self, data_sample, 
        ...
        for col in range(self.docByTopics.shape[1]) :
            ...
            for n in range(1, top_n+1) :
                ...
                result_df = pd.concat([result_df, pd.DataFrame([DICT])], ignore_index=1)
    ...
    def save(self, directory, file_name, data) :
        ...
        pd.DataFrame(self.get_docByTopics()).to_excel(writer , sheet_name = 'docByTopics', index = 1)
        pd.DataFrame(self.get_topicProportion()).to_excel(writer , sheet_name = 'topic_proportion', index = 1)
        pd.DataFrame(self.get_topwordByTopics()).to_excel(writer , sheet_name = 'topwordByTopics', index = 1)
        self.get_most_similar_doc2topic(data, title = 'title', date = 'year_application').to_excel(writer , sheet_name = 'titleByTopics', index = 1)
        pd.DataFrame(self.get_wordByTopics()).to_excel(writer , sheet_name = 'wordByTopics', index = 1)
        
        self.topic_time_df.to_excel(writer , sheet_name = 'TVByTimes', index = 1)
        self.topic_time_df_summary.to_excel(writer , sheet_name = 'TVByTimes_summary', index = 1)
